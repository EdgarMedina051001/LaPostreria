...
import com.example.lapostreria.databinding.CarritoItemBinding
...
class CarritoAdapter(
    ...
    private val carritoItemIngrediente: MutableList<String>
    ...
    inner class CarritoViewHolder(private val binding: CarritoItemBinding) :
        ...
        fun bind(position: Int) {
            binding.apply {
                ...
                tvNombrePostreCarrito.text = carritoItems[position]
                tvPrecioPostreCarrito.text = carritoItemPrecio[position]
                val uriString = carritoItemImagen[position]
                val uri = Uri.parse(uriString)
                Glide.with(context).load(uri).into(civImagenPostreCarrito)
                tvCantidadPostreCarrito.text = cantidad.toString()
        ...
        private fun disminuirCantidad(position: Int) {
            if (itemCantidades[position] > 1) {
                ...
                binding.tvCantidadPostreCarrito.text = itemCantidades[position].toString()
            }
        }
        ...
        private fun incrementarCantidad(position: Int) {
            if (itemCantidades[position] < 10) {
                itemCantidades[position]++
                carritoItemCantidad[position] = itemCantidades[position]
                binding.tvCantidadPostreCarrito.text =
        ...
        private fun removerItem(position: Int, llaveUnica: String) {
            if (llaveUnica != null) {
                carritoItemsRef.child(llaveUnica).removeValue().addOnSuccessListener {
                    ...
                    carritoItemIngrediente.removeAt(position)
